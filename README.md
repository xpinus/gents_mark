...
> ⚠️ **重要提醒**：卸载扩展会清除扩展自己的全部存储（包括 `chrome.storage.sync` 同步数据），金库书签会永久丢失。卸载前请务必**导出备份文件**；云端同步只用于在“仍保持安装的设备”之间共享数据，不能作为卸载后的备份。
...
- 手动触发：加密金库文档分块写入 `chrome.storage.sync`，规避单条 8KB 配额
- 自动拉取：打开面板时会读取云端数据；本机没有金库时直接恢复，同一主密码时自动合并新增条目
- 密码与会话密钥永不进入同步；其他设备需安装同一扩展、登录同一 Edge 账号并输入同一主密码后才能解锁
- 若两台设备主密码不同，不会强行合并，界面会提示先统一主密码
- 配额限制：分块后总量约 90KB 以内可同步，超出时提示用户；适合中小规模书签库
- Edge 要求：需登录 Edge 账号，并在 设置 > 个人资料 > 同步 中开启“扩展”同步
- 删除设备上的条目不会生成墓碑，合并后可能被云端旧副本重新带回；删除请在两端同步完成后操作
